...
fun BirthdayScreen(
  ...
  val pickerSpecificColors =
    currentAppColors.copy(
  ...
  // Initial Dialog
  Dialog(showDialog = showDialog, onDismissRequest = { /* Consider behavior */}) {
    Column(
      modifier =
      ...
      Button(
        ...
        shape = CircleShape,
        colors = ButtonDefaults.buttonColors(backgroundColor = Color(0xFFAAE0FA)),
  ...
  if (!showDialog) {
    Column(
      modifier =
        Modifier.fillMaxSize().padding(start = 25.dp, end = 25.dp, top = 4.dp, bottom = 10.dp),
